...
#[tokio::test]
async fn deploy_halt_option_defaults_off_toggles_and_resets_on_reopen() {
    registry_env();
    for (keys, expected) in [
        (vec![KeyCode::Char('D')], false),
        (vec![KeyCode::Char('D'), KeyCode::Char('h')], true),
        (
            vec![
                KeyCode::Char('D'),
                KeyCode::Char('h'),
        ...
        assert_eq!(confirm.halt_on_build_failure, expected);
    }
}
